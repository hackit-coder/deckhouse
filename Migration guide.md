...
| DropApp | Вариант поставки K8s, который включается в себя его основные компоненты, утилиты для развёртывания, CRI и набор операторов |
...
| CRI-O | Вариант исполнения (container runtime interface) помимо<b>docker</b>и containerd. Используется в DropApp в качествеобработчика контейнеров. Позволяет загружать образы контейнеров из репозиториев, управлять ими и запускать их |
...
| Workloads (рабочая нагрузка) | Абстрактная модель группы pods в K8s. DropApp подразделяет<b>workloads</b>на<b>Deployments</b>,<b>StatefulSets</b>,<b>DaemonSets</b>, задачи (jobs) и плановые задачи (cron jobs) |
...
| Annotations (аннотации) | Позволяют ассоциировать с ресурсами произвольные метаданные, менять свойства или поведение объектов DropApp, а также настраивать их взаимодействие с другими сервисами DropApp |
| Controller (контроллер) | Процесс в DropApp, который следит за состоянием ресурсов кластера K8s и, в случае, если текущее состояние отличается от заданного (желаемого), пытается привести состояние кластера к желаемому. |
